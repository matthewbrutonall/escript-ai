...
const actions = {
    ...
    async fetchAiBackends({ commit }) {
        try {
            const { data } = await retrieveAiBackends();
            const list = Array.isArray(data) ? data : (data?.results || []);
            commit("setAiBackends", list);
        } catch (error) {
            commit("setAiBackends", []);
        }
    },
};
...
const mutations = {
    ...
    setAiBackends(state, backends) {
        state.aiBackends = backends;
    },
