...
const actions = {
    ...
    async transcribeDocument({ rootState }, { documentId, parts }) {
        ...
        if (data) {
            ...
            const selected = rootState?.forms?.transcribe?.model || "";
            if (String(selected).startsWith("ai:")) {
                await transcribeDocumentAI({
                    documentId,
                    backend: String(selected).slice(3),
                    transcription,
                    parts,
                });
            } else {
