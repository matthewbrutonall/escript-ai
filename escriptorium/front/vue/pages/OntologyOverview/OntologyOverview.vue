            ...
            <div class="escr-container">
                <div class="escr-ontology-overview-header">
                    <h3 :title="documentName">
                        {{ documentName || $t("ontology.loading") }}
                    </h3>
                ...
                <div class="escr-ontology-overview-controls">
                    ...
                    >
                        <h3>{{ $t("ontology.transcription") }}</h3>
                        <EscrDropdown
                            :label="$t('ontology.transcriptionHelp')"
                            :disabled="loading"
                ...
                >
                    <div class="escr-ontology-overview-types">
                        ...
                        <EscrLoader
                            v-else
                            :loading="false"
                            :no-data-message="$t('ontology.empty')"
                        />
                    </div>
                    <div class="escr-ontology-overview-parts">
                        <template v-if="selectedType">
                            <h2>
                                {{ $t("ontology.partsContaining", { name: selectedType.name }) }}
                            </h2>
                            <EscrTable
                            ...
                            <EscrLoader
                                ...
                                :no-data-message="$t('ontology.unused')"
                            />
                        </template>
                        <p v-else>
                            {{ $t("ontology.selectType") }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    computed: {
        ...
        breadcrumbs() {
            ...
            return [
                ...
                ...docBreadcrumbs,
                { title: this.$t("ontology.title") },
            ];
        },
        categories() {
            return [
                { label: this.$t("ontology.regions"), value: "regions" },
                { label: this.$t("ontology.lines"), value: "lines" },
                { label: this.$t("ontology.textAnnotations"), value: "text" },
                { label: this.$t("ontology.imageAnnotations"), value: "image" },
                { label: this.$t("ontology.characters"), value: "characters" },
            ].map((category) => ({
                ...category,
        ...
        typesHeading() {
            const headings = {
                regions: this.$t("ontology.regionTypes"),
                lines: this.$t("ontology.lineTypes"),
                text: this.$t("ontology.textTypes"),
                image: this.$t("ontology.imageTypes"),
                characters: this.$t("ontology.characters"),
            };
            return headings[this.category] || this.$t("ontology.title");
        },
        typeHeaders() {
            return [
                {
                    label: this.category === "characters"
                        ? this.$t("ontology.characters")
                        : this.$t("ontology.type"),
                    value: "name",
                    sortable: false,
                },
                { label: this.$t("ontology.countInDocument"), value: "frequency", sortable: false },
            ];
        },
        partHeaders() {
            return [
                { label: this.$t("ontology.part"), value: "part_name" },
                { label: this.$t("ontology.countOnPart"), value: "frequency" },
            ];
        },
        selectedTypeKey() {
