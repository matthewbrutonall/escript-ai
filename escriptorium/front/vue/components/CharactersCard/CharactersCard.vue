<template>
    <div :class="classes">
        <div class="escr-card-header">
            <h2>{{ label || $t("characters.title") }}</h2>
        ...
        <EscrLoader
            v-else
            :loading="loading"
            :no-data-message="$t('characters.empty')"
        />
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        sortOptions() {
            return [
                { label: this.$t("characters.byCharacter"), value: "char" },
                { label: this.$t("characters.byFrequency"), value: "frequency" },
            ].map((sort) => ({
                ...sort,
