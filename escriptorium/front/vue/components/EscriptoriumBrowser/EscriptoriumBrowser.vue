    ...
    <div class="escr-browser escr-card">
        <div class="escr-card-header">
            <h3 class="escr-browser-breadcrumbs">
                <span v-if="!currentProject">{{ $t("browser.projects") }}</span>
                <template v-else>
                    <a
                        href="#"
                        @click.prevent="fetchProjects"
                    > {{ $t("browser.projects") }} </a>
            ...
            <div class="escr-browser-filters">
                <FilterSet
                    ...
                    :search-placeholder="
                        currentDocument ? $t('browser.searchImages') :
                        currentProject ? $t('browser.searchDocuments') :
                        $t('browser.searchProjects')
                    "
                    :tags="currentTags"
        ...
        >
            ...
            <div v-if="!currentProject && !currentDocument">
                <EscrLoader
                    ...
                    :loading="loading"
                    :no-data-message="$t('browser.emptyProjects')"
                />
                <template v-else>
                    <EscrTable
                        item-key="id"
                    ...
                    >
                        <div
                            ...
                            role="status"
                        >
                            <span class="sr-only">{{ $t("browser.loadingMore") }}</span>
                        </div>
                    </div>
                </template>
            </div>
            ...
            >
                <EscrLoader
                    ...
                    :no-data-message="$t('browser.emptyDocuments')"
                />
                <template v-else>
                    <EscrTable
                        item-key="pk"
                ...
                >
                    <div
                        ...
                        role="status"
                    >
                        <span class="sr-only">{{ $t("browser.updating") }}</span>
                    </div>
                </div>
            </div>
            <!-- Images list -->
...
export default {
    ...
    computed: {
        ...
        projectHeaders() {
            return [
                { label: this.$t("browser.projectName"), value: "name", sortable: true, class: "col-title" },
                { label: this.$t("common.tags"), value: "tags", component: EscrTags, class: "col-tags" },
                {
                    label: this.$t("browser.documents"),
                    value: "documents_count",
                    sortable: true,
                    class: "col-small",
                },
                {
                    label: this.$t("browser.updatedAt"),
                    value: "updated_at",
        ...
        documentHeaders() {
            return [
                { label: this.$t("browser.documentName"), value: "name", sortable: true, class: "col-title" },
                { label: this.$t("common.tags"), value: "tags", component: EscrTags, class: "col-tags" },
                { label: this.$t("browser.pages"), value: "parts_count", sortable: true, class: "col-small" },
                {
                    label: this.$t("browser.updatedAt"),
                    value: "updated_at",
                    format: this.formatDate,
