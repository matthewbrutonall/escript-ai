<template>
    <EscrModal class="escr-moveimages-modal">
        <template #modal-header>
            <h2>{{ selectedParts.length > 1 ? $t("move.titleMany") : $t("move.titleOne") }}</h2>
        </template>
        <template #modal-content>
            {{ $t("move.body", { n: selectedParts.length }) }}
            <h3>
                {{ $t("move.location") }}<span>
                    aria-label="required"
                    class="escr-required"
                >*</span>
            </h3>
            ...
            <label class="escr-text-field">
                <input
                    ...
                    min="1"
                    :max="partsCount"
                    :placeholder="$t('move.number')"
                    :disabled="disabled"
                    :value="index"
                    @input="(e) => handleChange('index', e.target.value)"
                >
                <span>{{ $t("move.of", { n: partsCount }) }}</span>
            </label>
        </template>
        <template #modal-actions>
            ...
            >
                ...
                />
                <template #popper>
                    {{ $t("move.invalid", { n: partsCount.toString() }) }}
                </template>
            </VDropdown>
            <EscrButton
                v-else
...
export default {
    ...
    computed: {
        ...
        locationOptions() {
            return [
                {
                    label: this.$t("move.before"),
                    value: "before",
                    selected: this.location === "before",
                },
                {
                    label: this.$t("move.after"),
                    value: "after",
