...
- **Phase 4 — AI segmentation review (§6), multi-tenant hardening (§12), more providers.** *In tree now (seg review):* numbered-box VLM pass stores `AISegSuggestion` (spurious / missed / order / typology) on existing lines; accept/dismiss on the document dashboard; never overwrites `Line.mask`. Multi-tenant and extra providers still later.
