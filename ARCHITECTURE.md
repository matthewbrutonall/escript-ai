...
- **Phase 4 — AI segmentation review (§6), multi-tenant hardening (§12), more providers.** *In tree now:* numbered-box review + geometry spurious flags; Accept deletes overlapping lines and stamps types. Monthly USD cap (`AI_MONTHLY_BUDGET_USD` / `AIUserQuota`) and per-user Fernet keys (`AIUserKey`, precedence over env). Extra providers still later.
