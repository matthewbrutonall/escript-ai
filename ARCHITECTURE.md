...
4. **Azure OpenAI** (deployment name in `model_id`, resource URL in `endpoint`) and **Mistral/Pixtral** — same chat-completions helper as local.
...
- **Phase 2 — the loops + gate:** dual-engine disagreement triage, non-skippable sample, layer state machine, `ketos test` CER (§9); style priming + conventions object (§10.1). *In tree now:* conventions toggles on the prompt; CER vs an existing comparison layer, or a random AI-line sample if none; `AILayerGate` (`raw → sampled → training-eligible`); admin/API/document-dashboard sample review; training collection picks omit ineligible AI layers; `TrainSerializer`/`core.tasks.train` refuse raw/sampled AI layers. *In tree now (continued):* same-document text few-shot; cheap kraken comparison pass when no sibling layer exists (non-fatal); held-out reviewed pages excluded from `train`; `ketos test` hook if the binary is present. Image few-shot and embedding retrieval are still later.
- **Phase 3 — hardening:** interactive "fix this" (§10.2); Passim fallback (§5.C); robustness on numbered-assignment failure modes. *In tree now:* colour-key assignment that is half-empty or scrambled falls back to per-line crops, then a witness→OCR snap that never copies kraken into gaps; `POST .../ai_fix/` re-reads one line with optional guide text from the sample-review table.
- **Phase 4 — AI segmentation review (§6), multi-tenant hardening (§12), more providers.** *In tree now:* numbered-box review + geometry spurious flags; Accept deletes overlapping lines and stamps types. Monthly USD cap (`AI_MONTHLY_BUDGET_USD` / `AIUserQuota`) and per-user Fernet keys (`AIUserKey`, precedence over env). Azure OpenAI and Mistral/Pixtral are in the backend registry.
